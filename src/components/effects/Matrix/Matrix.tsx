  ...
  useEffect(() => {
    ...
    class Drop {
      ...
      draw(ctx: CanvasRenderingContext2D) {
        ...
        // * Draw trail with solid colors (optimized)
        // Disable shadowBlur for trails to improve performance
        ctx.shadowBlur = 0;
        this.trail.forEach((trailItem, index) => {
          const trailOpacity = (index / this.trail.length) * this.opacity * 0.3;
          ctx.fillStyle = `rgba(0, 200, 0, ${trailOpacity * 0.8})`;
          ctx.fillText(trailItem.char, this.x, trailItem.y * this.fontSize);
        });

        // * Draw main character with enhanced effects
        // Use solid fill style instead of linear gradient for performance
        if (this.brightness) {
          ctx.fillStyle = `rgba(255, 255, 255, ${this.opacity * 1.5})`;
          ctx.shadowColor = "rgba(255, 255, 255, 0.9)";
          // Reduced shadowBlur for performance while keeping the effect
          ctx.shadowBlur = 8;
        } else {
          ctx.fillStyle = `rgba(0, 255, 100, ${this.opacity})`;
          // Only apply shadow blur to brighter non-white characters
          if (this.opacity > 0.7) {
    ...
    const drawFrame = (currentTime: number) => {
      ...
      if (currentTime - lastTime >= frameInterval) {
        ...
        // * Vignette overlay is handled via CSS for better performance
