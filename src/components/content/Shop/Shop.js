...
import {
  createPrintfulHeaders,
  getPrintfulConfig,
} from "../../../utils/printfulConfig";
import {
  handlePrintfulError,
  parsePrintfulProduct,
} from "../../../utils/printfulHelpers";
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
          ...
          validProducts.map(async (product) => {
            try {
              const detailResponse = await axios.get(
                `/store/products/${product.id}?store_id=${storeId}`,
                {
                  headers: createPrintfulHeaders(apiKey),
                },
              );
        ...
        const validProductsWithDetails = productsWithDetails.filter(
          (product) => {
        ...
        <div className="shop-content">
          {products.length === 0 ? (
            ...
            products.map((product, index) => {
              ...
              const { syncProduct, firstVariant, price } =
                parsePrintfulProduct(product);
