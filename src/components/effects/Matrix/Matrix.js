...
const Matrix = ({ isVisible, onSuccess }) => {
	...
	const handleKeyDown = useCallback((e) => {
		if (e.key === "Escape") {
			onSuccess?.();
		} else if (e.key === "Enter" && !showIncorrectFeedback && !showSuccessFeedback) {
			handleSubmit(e);
		...
		>
			...
			>
				...
				<div className="hint-speech-txt">
					...
					>
						<span className="hint-text">Digital whispers echo through the void...</span>
					...
					{hintLevel < 2 && (
						<div className="hint-prompt">
							{hintLevel === 0 ? "Click for more..." : "One more secret remains..."}
						</div>
					)}
				</div>
