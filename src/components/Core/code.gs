...
function handleRequest(e) {
  try {
    ...
    switch(e.parameter.action) {
      ...
      case 'createBingoSheet':
        return createBingoSheet(data.year);
      default:
...
// Helper function to create a new bingo sheet
function createBingoSheet(year) {
  const ss = SpreadsheetApp.openById(SPREADSHEET_ID);
  const tabName = `bingo${year}`;
  ...
  if (sheet) {
    return {
      success: true,
      message: `Sheet ${tabName} already exists`,
  ...
  // Set up headers
  const headers = ['Check', 'Category', 'Goal', 'Description', 'ID'];
  sheet.getRange(1, 1, 1, headers.length).setValues([headers]);
  ...
  // Set column widths
  sheet.setColumnWidth(1, 70);  // Check
  sheet.setColumnWidth(2, 100); // Category
  sheet.setColumnWidth(3, 300); // Goal
  sheet.setColumnWidth(4, 400); // Description
  sheet.setColumnWidth(5, 100); // ID
  
  // Add data validation for Check column (0 or 1)
  const checkValidation = SpreadsheetApp.newDataValidation()
    .requireValueInList(['0', '1'], true)
    .setAllowInvalid(false)
    .build();
  sheet.getRange('A2:A26').setDataValidation(checkValidation);
  
  // Add data validation for Category column
  const categories = ['Health', 'Career', 'Personal', 'Social', 'Finance'];
  const categoryValidation = SpreadsheetApp.newDataValidation()
    .requireValueInList(categories, true)
    .setAllowInvalid(false)
    .build();
  sheet.getRange('B2:B26').setDataValidation(categoryValidation);
  
  // Initialize 25 rows with default values
  const defaultRows = Array(25).fill(['0', '', '', '', '']);
  sheet.getRange(2, 1, 25, 5).setValues(defaultRows);
  
  // Add conditional formatting for checked items
  const checkedRule = SpreadsheetApp.newConditionalFormatRule()
    .whenTextEqualTo('1')
    .setBackground('#c2e0c6')
    .setRanges([sheet.getRange('A2:A26')])
    .build();
  sheet.setConditionalFormatRules([checkedRule]);
  ...
  protection.setDescription('Sheet Structure Protection');
  protection.setWarningOnly(true);
  
  return {
    success: true,
    message: `Sheet ${tabName} created successfully`,
    sheetId: sheet.getSheetId()
  };
}